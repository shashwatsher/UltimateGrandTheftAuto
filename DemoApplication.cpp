...
double startTime, startTimeAfterInc=0.0;
...
m_cameraCanonPosition(1.f,1.f,1.f),//0.2, 0.5, 1
m_mouseOldX(0),
m_mouseOldY(0),
m_mouseButtons(0),
m_modifierKeys(0),
...
void DemoApplication::keyboardCallback(unsigned char key, int x, int y)
{
	...
	/* case '.':
	{
	if ((m_cameraCanonPosition[0] != 0) && (m_cameraCanonPosition[1] != 0) && (m_cameraCanonPosition[2] != 0)){

	shootBox(getRayToCanon(x,y));//getCameraTargetPosition());
	}
	break;
	}*/
	case '+':
...
void	DemoApplication::setShootBoxShape ()
{
	...
	//{
		//btBoxShape* box = new btBoxShape(btVector3(0.5,0.5,0.5));
                ...
                btBoxShape* box2;
                if(m_harpoon){
                   box2 = new btBoxShape(btVector3(0.1,0.1,1));
                   m_shootBoxShape = box2;
                }else{
                   box = new btSphereShape(btScalar(0.2));
                   m_shootBoxShape = box;
                }
...
void	DemoApplication::shootBox(const btVector3& destination)
{
	...
	if (m_dynamicsWorld)
	{
		...
		btVector3 camPos = getCameraCanonPosition();
...
/************** new shit ************************
                ...
                ghostObject->setCollisionShape(m_shootBoxShape);
                btTransform localTrans;
	        localTrans.setIdentity();
		localTrans.setOrigin(btVector3(0, 1, 0));// 0 1 0
      ghostObject->setWorldTransform(localTrans);
      ...
      m_dynamicsWorld->getBroadphase()->getOverlappingPairCache()->setInternalGhostPairCallback(new btGhostPairCallback());   
               ...
               //m_dynamicsWorld->getPairCache()->setInternalGhostPairCallback(ghostPairCallback);
                btManifoldArray   manifoldArray;
                
btBroadphasePairArray& pairArray = m_dynamicsWorld->getBroadphase()->getOverlappingPairCache()->getOverlappingPairArray();
    ...
    for (int j=0;j<manifoldArray.size();j++)
    {
        ...
        for (int p=0;p<manifold->getNumContacts();p++)
        {
            ...
            {   
                ...
                btVector3 End;
                btVector3 Normal;
                btCollisionWorld::ClosestRayResultCallback RayCallback(m_cameraCanonPosition, destination);
                m_dynamicsWorld->rayTest(m_cameraPosition,destination,RayCallback);
...
/******************************** new shit ends **************************************/
		...
		//body->getWorldTransform().setOrigin(camPos);
                body->getWorldTransform().setOrigin(m_cameraCanonPosition);
		body->getWorldTransform().setRotation(btQuaternion(0,0,0,1));
		body->setLinearVelocity(linVel);
		body->setAngularVelocity(btVector3(0,0,0));
		body->setCcdMotionThreshold(0.0);//0.5
		body->setCcdSweptSphereRadius(100.f);//0.4//value should be smaller (embedded) than the half extends of the box (see ::setShootBoxShape)
...
/// *********** Canon Code 
btVector3 DemoApplication::getRayToCanon(int x,int y)
{
	...
	if (m_ortho)
	{
		...
		extents *= m_cameraDistance;
		btVector3 lower = m_cameraCanonPosition - extents;
		btVector3 upper = m_cameraCanonPosition + extents;

		btScalar u = x / btScalar(m_glutScreenWidth);
		btScalar v = (m_glutScreenHeight - y) / btScalar(m_glutScreenHeight);

		btVector3	p(0,0,0);
		p.setValue((1.0f - u) * lower.getX() + u * upper.getX(),(1.0f - v) * lower.getY() + v * upper.getY(),m_cameraCanonPosition.getZ());
		return p;
	}
	...
	btVector3 rayForward = (getCameraCanonPosition()-getCameraPosition());
...
void DemoApplication::mouseFunc(int button, int state, int x, int y)
{
	...
	case 2:
	{
		...
		{
			soundEngine->play2D("shooting_sound.wav",false);
			if ((m_cameraCanonPosition[0] != 0) && (m_cameraCanonPosition[1] != 0) && (m_cameraCanonPosition[2] != 0)){
...
void DemoApplication::showProfileInfo(int& xOffset,int& yStart, int yIncr)
{
...

	{
		...
		if (executionTime <= 10)
		{
			sprintf(blockTime, "Loading.............");
			displayProfileString(300, 200, blockTime);
		}
		else
		{
			
			gameStarted = true;
			if (executionTime > m_total_time)//120
			{
				gameStarted = false;
				sprintf(blockTime, "	GAME OVER");
				displayProfileString(300, 200, blockTime);
				yStart += 20;
				sprintf(blockTime, "	Score : %d", counter);
				displayProfileString(xOffset, yStart, blockTime);
			}
			else
			{
				sprintf(blockTime, "Total Running Time : %1.0f s", executionTime);
				displayProfileString(xOffset, yStart, blockTime);
				yStart += 20;
				sprintf(blockTime, "Score : %d", counter);
				displayProfileString(xOffset, yStart, blockTime);
				yStart += 20;
				sprintf(blockTime, "Speed : %f", m_speed);// Speed Check
				displayProfileString(xOffset, yStart, blockTime);
				if (counter == 80 && !timeExtended)
				{
					m_total_time += 30;
					timeExtended = true;
					if (startTimeAfterInc == 0.0)
					{
						startTimeAfterInc = clock();
					}	
				}
				double endTimeAfterInc = clock();
				double executionTimeAfterTimeInc = (double)(endTimeAfterInc - startTimeAfterInc) / CLOCKS_PER_SEC;
				if (executionTimeAfterTimeInc < 5)
				{
					yStart += 20;
					sprintf(blockTime, "		30 Seconds added");// Time Increase
					displayProfileString(xOffset, yStart, blockTime);
				}
			}
...
void DemoApplication::renderme()
{
	...
	if (m_dynamicsWorld)
	{
		...
		if ((m_debugMode & btIDebugDraw::DBG_NoHelpText)==0)
		{
			...
			{
				// RAHUL COMMENTED static int counter = 0;
				counter++;
